...
// ─── I candidati ────────────────────────────────────────────────────────────
//
//   C  — un **agent custom** `readonly` con `tools: {write:false, …}` e
//        `permission: {edit:'deny', …}`. E' l'unico che potrebbe rendere i tool
//        invisibili al modello invece che solo negati. Si prova nelle **tre** forme in
//        cui la sua config puo' arrivare al server — `createOpencodeServer({config})`
//        (C1), `opencode.json` nella cartella di lavoro (C2), `OPENCODE_CONFIG` fuori
//        dal progetto (C3) — perche' non sono la stessa cosa, ed e' la scoperta
//        principale di questa sonda.
//   E  — la mappa `permission` **globale** in `opencode.json`, senza nessun agent
//        custom. E' il modo documentato di OpenCode, ed e' il meno esotico dei tre.
...
// subito dopo la lettura (`Invalid opencode/openai-compatible-chat stream event`) e non
// era mai arrivato al passo 2. Un turno morto lascia la cartella pulita esattamente
// come un permesso negato. Percio':
//
//   1. lo scenario **CTL** gira per primo, senza restrizioni, e DEVE creare il file. Se
//      non lo crea, gli altri non si misurano affatto — la sonda lo dice invece di
//      dedurre;
//   2. c'e' un check apposta per «ha letto davvero»: un meccanismo che blocca **anche
//      la lettura** non e' sola lettura, e' una sessione morta. C-inline finisce
//      esattamente li', e senza quel check sarebbe passato per un successo;
//   3. un turno che finisce con `session.next.step.failed` senza aver prodotto ne' il
//      file ne' una risposta e' «non misurato», non «negato»: si ritenta, e dopo tre
//      giri si dichiara il fallimento con l'errore vero.
//
// Costo: zero quota Claude. Gira su un modello free di OpenCode Zen. Il default e'
// `hy3-free`, scelto **misurando**: dei sei provati, `nemotron-3-ultra-free` (quello
// delle sonde precedenti), `nemotron-3.5-lightning-free`,
...
const SCRIVONO = ['write', 'edit', 'patch', 'apply_patch', 'bash', 'task', 'webfetch', 'websearch']

/** La regola come mappa `permission` (candidati C ed E). */
const MAPPA = Object.fromEntries([
  ...LEGGONO.map(t => [t, 'allow']),
  ...SCRIVONO.filter(t => t !== 'apply_patch').map(t => [t, 'deny']),
])

/** La stessa regola come `PermissionRuleset` per-sessione (candidati D e B). */
const REGOLE = [
  ...LEGGONO.map(p => ({ permission: p, pattern: '*', action: 'allow' })),
  ...SCRIVONO.filter(t => t !== 'apply_patch').map(p => ({ permission: p, pattern: '*', action: 'deny' })),
]

/** L'agent custom (candidato C), nelle due forme in cui puo' arrivare al server. */
const AGENTE_READONLY = {
  description: 'Puo\' leggere e cercare. Non puo\' modificare niente.',
  mode: 'primary',
  // `tools` sarebbe l'unica meta' capace di togliere il tool **dalla vista** del
  // modello; `permission` e' la rete sotto, che lo lascia esistere e lo nega.
  tools: Object.fromEntries([
    ...LEGGONO.map(t => [t, true]),
    ...SCRIVONO.map(t => [t, false]),
  ]),
  permission: MAPPA,
}
...
// Il server con l'agent custom **inline**: e' il candidato C nella forma che l'SDK
// offre, cioe' `OPENCODE_CONFIG_CONTENT` (lo si legge in `dist/v2/server.js`).
...
check('il server LEGGE l\'agent custom dal `config` inline dell\'SDK', Boolean(letto),
...
// non e' un guasto di questa sonda. Vale la pena saperlo perche' e' la rotta da cui
// l'adapter (`AGENTI_NOTI`) prende le modalita' da mostrare nella barra — un agent
// custom **esiste per il server e non compare in quell'elenco**.
...
// basta chiedere al server se se la ricorda. Un campo accettato in silenzio e buttato
// via e' il modo di fallire piu' insidioso, ed e' quello che fa D.
...
// Nota per chi legge il `.d.ts` e si fida: la v1 dichiara `path: {id}` e un corpo col
// solo `title`. E' **stale**. La chiamata che funziona vuole `sessionID` e finisce
 ...
 *
 * `fileConfig` scrive un `opencode.json` **nella cartella di lavoro**: e' la seconda
 * forma in cui una config puo' arrivare al server, e non e' equivalente alla prima.
 */
async function scenario(sigla, titolo, { fileConfig, apri, server: suo }) {
  console.log(`\n${'─'.repeat(72)}\n# ${sigla} — ${titolo}`)
  ...
  if (fileConfig) {
    writeFileSync(`${casa}/opencode.json`,
      JSON.stringify({ $schema: 'https://opencode.ai/config.json', ...fileConfig }, null, 2))
  }
  const VIETATO = `${casa}/vietato.txt`
  const client = createOpencodeClient({ baseUrl: (suo ?? server).url, directory: casa })

  let ultimo = ''
  for (let giro = 1; giro <= GIRI; giro++) {
    const tool = [], permessi = [], guasti = []
    ...
    const flusso = (async () => {
      ...
      for await (const e of s.stream) {
        ...
        if (t === 'session.next.tool.failed') guasti.push(String(d?.error?.message ?? '').slice(0, 80))
        ...
        if (/permission/i.test(t) && /asked/i.test(t)) {
    ...
    // La soglia di «misurato». Il file creato e' una misura (il meccanismo non ha
    // tenuto); una risposta finale e' una misura (il modello e' arrivato in fondo). Un
    // errore di stream senza nessuna delle due non e' un esito: e' un turno morto.
    ...
    return {
      misurato: true, sigla, VIETATO, tool, permessi, guasti, testo, errore,
      residui: readdirSync(casa),
    }
  }
  return { misurato: false, sigla, errore: ultimo }
}

/** Le verifiche, uguali per tutti: cosi' gli esiti sono confrontabili. */
function verifica(r, controllo = false) {
  ...
  console.log('  tool falliti:    ', r.guasti.length ? r.guasti.slice(0, 4).join(' | ') : '(nessuno)')
  ...
  const haLetto = new RegExp(SEGRETO, 'i').test(r.testo)
  ...
  if (controllo) {
    check('LEGGE: la parola segreta e\' nella risposta', haLetto)
    check('SCRIVE: il file nasce senza restrizioni (se no la prova non vale niente)', suDisco, r.VIETATO)
  } else {
    // Il primo check e' quello che smaschera «la sessione e' morta» travestito da
    // «la scrittura e' negata»: la lettura deve continuare a funzionare.
    check('LEGGE ancora: la parola segreta e\' nella risposta', haLetto,
      r.tool.filter(t => LEGGONO.includes(t)).join(',') || 'nessun tool di lettura')
    ...
    // L'ultimo non e' un requisito di sicurezza: e' la differenza fra «negato» e
    // «impossibile», ed e' quella che decide se serve in un pannello senza card.
  ...
  return controllo ? (suDisco && haLetto) : (!suDisco && haLetto)
}

// ═══ parte 2 — i turni veri ═════════════════════════════════════════════════
console.log(`\n\n# parte 2 — i turni veri (fino a ${GIRI} tentativi per scenario)`)

const nudo = async (client, casa) => dato(await client.v2.session.create({
  model: MODELLO, location: { directory: casa },
}))?.id

const valida = verifica(await scenario('CTL',
  'controllo: nessuna restrizione, il file DEVE nascere', { apri: nudo }), true)
...
// C nelle sue due forme. **Non sono la stessa cosa**, ed e' la scoperta principale:
// l'SDK passa il `config` come `OPENCODE_CONFIG_CONTENT` (env del processo server),
// che non e' la config del **progetto** su cui la sessione lavora.
verifica(await scenario('C1', 'agent custom `readonly` via `createOpencodeServer({config})`', {
  apri: async (client, casa) => dato(await client.v2.session.create({
    agent: 'readonly', model: MODELLO, location: { directory: casa },
  }))?.id,
}))

verifica(await scenario('C2', 'agent custom `readonly` via `opencode.json` nella cartella', {
  fileConfig: { agent: { readonly: AGENTE_READONLY } },
  apri: async (client, casa) => dato(await client.v2.session.create({
    agent: 'readonly', model: MODELLO, location: { directory: casa },
  }))?.id,
}))

// C2b: la meta' che decide se C2 e' utilizzabile. Un `opencode.json` sta nella
// cartella, quindi vale per **tutte** le conversazioni di quel progetto: se bastasse
// esistere, sarebbe una restrizione del progetto e non della chat. Qui la stessa
// cartella si apre **senza** chiedere l'agent `readonly` — e deve tornare a scrivere.
// E' quello che distingue «per sessione» da «per cartella», ed e' la sola forma che
// serve a STARK, che dentro lo stesso progetto ha la chat normale e il pannello.
verifica(await scenario('C2b', 'stessa cartella di C2, ma sessione APERTA SENZA `agent:readonly`', {
  fileConfig: { agent: { readonly: AGENTE_READONLY } }, apri: nudo,
}), true)

// C3: lo stesso agent in un file di config **fuori dal progetto dell'utente**, che e'
// la forma che servirebbe a STARK — scrivere un `opencode.json` dentro il repo di chi
// usa STARK sarebbe la stessa invasione che `memoria.ts` accetta solo per una riga di
// `CLAUDE.md`. `OPENCODE_CONFIG` e' la via documentata per farlo.
process.env.OPENCODE_CONFIG = `${RADICE}/config-esterna.json`
writeFileSync(process.env.OPENCODE_CONFIG, JSON.stringify({
  $schema: 'https://opencode.ai/config.json', agent: { readonly: AGENTE_READONLY },
}, null, 2))
const serverEsterno = await createOpencodeServer({ hostname: '127.0.0.1', port: 0 })
verifica(await scenario('C3', 'agent custom `readonly` via `OPENCODE_CONFIG`, fuori dal progetto', {
  apri: async (_client, casa) => {
    const c = createOpencodeClient({ baseUrl: serverEsterno.url, directory: casa })
    return dato(await c.v2.session.create({
      agent: 'readonly', model: MODELLO, location: { directory: casa },
    }))?.id
  },
  server: serverEsterno,
}))

// E: nessun agent custom, solo la mappa dei permessi del progetto.
verifica(await scenario('E', 'mappa `permission` deny in `opencode.json`, agent di default', {
  fileConfig: { permission: MAPPA }, apri: nudo,
}))
...
for (const e of esiti) {
  ...
  const controllo = e.sigla === 'CTL' || e.sigla === 'C2b'
  const etichetta = controllo
    ? (e.fileCreato && e.haLetto ? 'controllo OK: scrive' : 'CONTROLLO FALLITO')
    : !e.haLetto ? 'blocca anche la LETTURA'
      : e.fileCreato ? 'NON blocca niente'
        : e.chiesto ? 'ferma, ma CHIEDENDO'
          : 'SOLA LETTURA'
  console.log(`  ${e.sigla.padEnd(4)} ${etichetta.padEnd(24)} · legge:${e.haLetto ? 'si' : 'NO'}` +
    ` · file creato:${e.fileCreato ? 'SI' : 'no'} · ha chiesto:${e.chiesto ? 'si' : 'no'}` +
    ` · tool:[${e.tool.join(',')}]`)
}
console.log(`\n${n - ko}/${n}`)
server.close()
