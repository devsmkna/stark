// Il CLI di Claude Code accetta un testo **allegato al prompt**, o solo immagini?
//
...
const testo = process.argv[2] ?? '/tmp/prova-allegato.pdf'
const b64 = readFileSync(testo).toString('base64')
...
const msg = {
  ...
  message: {
    role: 'user',
    content: [
      { type: 'document', source: { type: 'text', media_type: 'text/plain', data: 'La parola nascosta e MELANZANA.' } },
      { type: 'text', text: 'Which single word is written in the attached testo? Answer with that word only, nothing else.' },
    ],
  },
...
console.log(risposta.toUpperCase().includes('MELANZANA')
  ? 'OK — il CLI ha passato il testo al modello, che lo ha letto'
  : 'NO — il testo non e\' arrivato leggibile')
