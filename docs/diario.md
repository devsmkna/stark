...
Cosa è successo, in che ordine, e **perché** è stato fatto così. Ogni voce è nata come
il resoconto di un giro di lavoro: il difetto, la misura che l'ha inchiodato, la cura, e
soprattutto le ipotesi che sembravano ottime e sono cadute.

## Perché sta qui e non in `CLAUDE.md`

Ci stava, ed era il 91% di quel file — **79.319 token misurati**, cioè il 76% di tutto
ciò che veniva caricato prima ancora che qualcuno scrivesse una parola. Un compito
qualunque, di quelle 169 voci, ne usa zero o una: era una struttura da **consultare**,
caricata come una da **leggere**.

Qui il costo è zero finché non serve, e quando serve si trova:

    grep -n "sticky\|overflow" docs/diario.md      # com'era stato risolto quel caso
    grep -n "26 agosto" docs/diario.md              # cosa è successo quel giorno

## Come si legge

In ordine cronologico, dalla voce più vecchia in fondo alla più recente. Le voci non si
riscrivono: quando una conclusione si rivela sbagliata, si aggiunge la correzione e si
lascia lì l'errore, perché la premessa su cui era stata presa una decisione vale quanto
la decisione. Diverse voci qui dentro sono esattamente questo — un'ipotesi coerente con
tutti i sintomi, misurata, e caduta.

Chi cerca **cosa** si è deciso (non com'è andata) sta guardando nel posto sbagliato: le
decisioni sono in `CLAUDE.md` («Decisioni già prese») e su Notion come ADR, con la
motivazione per esteso — ADR-003 dice dove vive cosa. Chi cerca com'è fatta una
schermata: `docs/ui-schermate.md`. Il modello di eventi: `docs/event-model.md`.

Buona parte di questo racconto vive anche nei **messaggi di commit** di questo repo, che
sono scritti lunghi apposta: `git log` è la stessa storia agganciata alle righe di codice
...
**Si rilascia con un tag, e chi ce l'ha installato lo viene a sapere** (28 agosto 2026,
chiesto dall'utente: «quando viene avviato il daemon fa un check su git se ci sono nuove
release, se ci sono aggiunge un banner in alto»). Prima `stark update` faceva
`git pull --ff-only`: **qualunque push su `main` arrivava a tutti**, e non c'era modo di
spingere un commit senza spedirlo a un collega nello stesso minuto. E nessuno sapeva che
esisteva un aggiornamento — l'unico modo era dirglielo a voce.

Adesso una release è un **tag `vX.Y.Z`** più la stessa versione in `package.json`, cioè
esattamente ciò che fa `npm version` in un colpo solo. Il daemon, accendendosi, chiede
al remoto quali tag esistono; se ce n'è uno più alto, in cima alla UI compare una banda
col bottone che aggiorna e il comando `stark update` accanto. Il processo sta in
`docs/rilascio.md`.

**Perché i tag e non le Release di GitHub**, che sarebbero «la cosa ufficiale»: qui il
confronto è fra due cose ufficiali, non fra una ufficiale e una fatta in casa — le
Release di GitHub *usano* i tag sotto. `git ls-remote --tags` è **un giro di rete e zero
oggetti scaricati**, passa dalle credenziali git che la macchina ha già (su un repo
privato l'API di GitHub vorrebbe un token in più da distribuire a ogni collega) e non
lega STARK a GitHub. Le note di rilascio restano possibili sopra, senza toccare niente.

**Il confronto è su `package.json`, non sui commit**, ed è quello che fa funzionare la
richiesta senza casi speciali: fra una release e l'altra `main` ha commit in più ma la
stessa `version`, quindi la risposta è «sei aggiornato» — che è la verità, perché non
c'è nessuna *release* più nuova. Ne segue una regola da non rompere: **la versione si
alza nel commit che porta il tag, e mai prima.** Ricaduta gradita e non cercata: su una
copia di sviluppo il banner non compare, quindi chi lavora al progetto non viene
invitato a buttare via il proprio ramo.

**Tre difetti che solo l'esecuzione dal vivo poteva far uscire**, e nessuno dei tre si
vedeva leggendo il codice.

1. **`git checkout` su un albero sporco non rifiuta.** Sembrava la difesa naturale — si
   provava a spostare il repo e git avrebbe detto di no. Misurato: con un file tracciato
   modificato e una release che tocca altri file, `git checkout --detach` **passa**, e si
   porta dietro la modifica in silenzio. La copia finirebbe su un tag *quasi* uguale a
   quello pubblicato, senza che nessuno sappia più in cosa differisce. La guardia è
   quindi nostra e sta **dentro `passaAllaRelease`**, dove nessun chiamante può
   saltarla — stessa ragione per cui `isDir` è finita dentro `registry.open()`.
2. **`npm install` sporca l'albero a ogni esecuzione**, e l'aggiornamento si chiudeva la
   porta alle spalle: riscrive `package-lock.json` (ci tiene dentro la `version`) e
   **anche `yarn.lock`**. Il primo aggiornamento riusciva, il secondo si rifiutava per
   «modifiche locali» che non erano di nessuno. Da qui `riallinea()`, che rimette i file
   tracciati com'erano dopo l'installazione. Buttarle via è sicuro **solo lì** e per un
   motivo preciso: `passaAllaRelease` si rifiuta di partire su un albero sporco, quindi
   quando si arriva a quel punto la partenza era pulita per costruzione e tutto ciò che
   è cambiato l'abbiamo cambiato noi. A/B deliberato: stesso giro con e senza la
   chiamata → `M yarn.lock` contro albero vuoto, e il secondo aggiornamento che riesce
   invece di rifiutarsi.
3. **Il ricambio moriva col cgroup dell'unità systemd** — ed era un difetto **che c'era
   già**, sul riavvio dalle impostazioni, non introdotto qui. `avviaRicambio` faceva
   `spawn(detached)`: `detached` stacca dal terminale (`setsid`) ma **non** dal cgroup,
   che un figlio eredita. Il daemon vive in `stark-<impronta>.service`; appena il suo
   processo principale esce, systemd ferma l'unità, e fermare un'unità vuol dire
   uccidere tutto ciò che resta nel suo cgroup — compreso il ricambio, che stava ancora
   contando i suoi quindici secondi. È la stessa malattia già documentata per gli scope
   di logind (27 agosto), in un altro vestito. Misurato in due modi: stesso giro col
   daemon **fuori** da systemd → aggiornamento a termine; dentro un'unità → il figlio
   sparisce senza scrivere una riga di log. E poi isolato dal prodotto, con un padre
   dentro un'unità che lancia un figlio nei due modi: `spawn(detached)` **non scrive
   niente**, `systemd-run` scrive. Il fix è un'unità transiente sua
   (`stark-ricambio-<impronta>`, nome diverso perché quella del daemon in quell'istante
   è ancora viva), cioè il meccanismo che systemd offre apposta — lo stesso già usato da
   `avviaConSystemd`. Nell'estrazione la lista delle variabili d'ambiente da passare a
   `systemd-run` ha smesso di esistere in due copie: era la stessa in `stark.ts` e nel
   ricambio, e una delle due un giorno perde una variabile (è già successo con
   `STARK_PUBLIC_HOST`, e si manifesta come un perimetro che si richiude in silenzio).

**Il bottone fa esattamente `stark update`**, non una procedura che gli somiglia: il
ricambio lancia quel comando. Due percorsi di aggiornamento diversi divergerebbero alla
prima correzione fatta su uno solo. Gira **dopo** che il vecchio è morto — un
`npm install` mentre il processo di prima è ancora vivo cambierebbe `node_modules` sotto
i piedi a chi lo sta usando — e con `|| true`, perché se l'aggiornamento fallisce STARK
deve tornare acceso **comunque**, sulla versione di prima: un aggiornamento che non
riesce è un fastidio, un daemon che non torna sono le conversazioni chiuse e nessuna
finestra da cui riaprirle.

**L'installer installa una release**, non la punta del ramo: il clone prende `main`
perché serve un punto da cui partire, poi `src/cli/release.ts checkout` sposta sul tag.
Quel file — e tutta la catena che importa — **non dipende da `node_modules`**, perché
gira *prima* di `npm install`: è una proprietà dichiarata in testa al file, e si
romperebbe solo sulla macchina di chi installa per la prima volta, cioè dove non la
vedremmo mai. Se non c'è ancora nessuna release lo **dice** e resta sul ramo.

Due cose che vanno sapute invece che scoperte. Il controllo lo fa il daemon
**all'accensione**, non la pagina a intervalli: un daemon lasciato acceso per una
settimana non si accorge di una release uscita nel frattempo — va bene perché aggiornare
riavvia, ma è il limite di questa scelta. E a ogni accensione parte un `git ls-remote`
verso il remoto del repo: è la seconda cosa che esce dalla macchina dopo il Web Push,
non manda niente (chiede quali tag esistono) ed è lo stesso posto da cui STARK è stato
installato, ma è traffico che prima non c'era.

Nello stesso giro, un difetto trovato **guardando lo screenshot** del giro vero e non il
codice: mentre l'aggiornamento gira il corpo della pagina diceva «The daemon is not
answering — start it again with `npm run stark:start`», cioè mandava a fare la cosa
sbagliata proprio mentre tutto funzionava, con la banda due righe sopra che diceva il
contrario. Ora lì c'è «Updating. Nothing to do».

Verificato dal vivo su un finto remoto con release vere (costo zero di quota): banda a
1400 e 390px con i rettangoli misurati — da telefono era alta **127px**, un sesto dello
schermo, e sfoltendo etichette e bottone come già fa la barra di stato è scesa a **49**;
clic → «Updating to 1.2.0…» → repo sul tag → `npm install` → UI ricompilata → daemon
tornato sotto la **sua** unità → pagina ricaricata da sé → banda sparita, il tutto in
